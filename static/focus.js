...
async function run() {
    ...
    const ogrnsAll = await Promise.all([
        sendRequest(`${API.orgReqs}?ogrn=${ogrns}`),
        sendRequest(`${API.analytics}?ogrn=${ogrns}`),
        sendRequest(`${API.buhForms}?ogrn=${ogrns}`)])
    const orgsMap = reqsToMap(ogrnsAll[0]);
    addInOrgsMap(orgsMap, ogrnsAll[1], "analytics");
    addInOrgsMap(orgsMap, ogrnsAll[2], "buhForms");
    render(orgsMap, orgOgrns);
}

run().catch(er => alert(er));
