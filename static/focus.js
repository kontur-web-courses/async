...
async function run() {
    try {
        ...
        let requisitesPromise = sendRequest(`${API.orgReqs}?ogrn=${ogrns}`);
        let analyticsPromise = sendRequest(`${API.analytics}?ogrn=${ogrns}`);
        let buhPromise = sendRequest(`${API.buhForms}?ogrn=${ogrns}`);
        [requisites, analytics, buh] = await Promise.all([requisitesPromise, analyticsPromise, buhPromise]);
