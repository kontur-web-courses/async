...
async function run() {
    ...
    const response1 = sendRequest(`${API.orgReqs}?ogrn=${ogrns}`);
    const response2 = sendRequest(`${API.analytics}?ogrn=${ogrns}`);
    const response3 = sendRequest(`${API.buhForms}?ogrn=${ogrns}`);
    const [requisites, analytics, buh] = await Promise.all([response1, response2, response3]);
